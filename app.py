######################################
#name: Ekaterina Prokopeva
#email: katerin@bu.edu
######################################
...
app.config['MYSQL_DATABASE_PASSWORD'] = 'antananarivuu1'
...
def login():
	...
	return render_template('hello.html', message='Oops, this email doesnt exist');
...
def picture_id():
	...
	if request.method == 'POST':
		...
		cursor = conn.cursor()
		cursor.execute("INSERT INTO Comments (content, owner, picture_id) VALUES ('{0}', '{1}', '{2}')".format(content, user_id, picture_id))
		conn.commit()
...
def getAlbumIdFromName(albumName, user_id):
	cursor = conn.cursor()
	cursor.execute("SELECT album_id  FROM Albums WHERE albumName = '{0}' AND user_id = '{1}'".format(albumName, user_id))
	return cursor.fetchone()[0]
...
def upload_file(albumName):
	if request.method == 'POST':
		...
		conn.commit()
		return render_template('profile.html', name=flask_login.current_user.id, message='Photo uploaded!', photos=getAlbumsPhotos(album_id)[0], album=albumName )
...
def mostPopularUsersTags():
	...
	cursor.execute("SELECT word FROM Tags, Pictures WHERE Tags.picture_id = Pictures.picture_id and user_id='{0}' GROUP BY (word) HAVING COUNT(*)>0 ORDER BY COUNT(*) DESC LIMIT 5".format(uid))
	T = cursor.fetchall()
...
def userActivity():
	...
	cursor.execute("SELECT Users.user_id, COUNT(picture_id) FROM Users, Pictures WHERE Users.user_id=Pictures.user_id GROUP BY Users.user_id ORDER BY COUNT(picture_id) DESC LIMIT 5")
	T = cursor.fetchall()
